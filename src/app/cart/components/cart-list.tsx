...
function CartListInner() {
  ...
  const onOrderHandler = async (formData: FormData) => {
    ...
    try {
      ...
      dispatch(
        addPopup({
          title: "Ошибка",
          data: "Произошла ошибка при создании заказа",
          type: "error",
        })
      );
    }
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-row justify-center items-center lg:items-start gap-5 lg:gap-20 w-full px-5">
      <div className="flex flex-col gap-5 lg:w-[660px] w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:w-[660px] w-full">
          {cart.map((product: CartProduct) => (
            <CartCard product={product} key={product.product.id} />
          ))}
        </div>
      </div>
      ...
      >
        <h3 className="text-2xl font-light w-full text-left">Корзина</h3>
        <div className="flex flex-col gap-1">
          {cart.map((item: CartProduct) => (
            <SideCartText product={item} key={item.product.id} />
          ))}
        </div>
        ...
        <InputBase label="Адрес доставки" name="adress" required />
        ...
        <div className="flex gap-5 flex-wrap">
          ...
          <button
            type="button"
            className="bg-white-900 px-5 py-3 rounded-md text-[16px] font-light whitespace-nowrap text-black hover:-translate-y-0.5 active:scale-95 transition-all cursor-pointer shadow-primary-800 hover:shadow-md border-2 border-solid border-primary-500 select-none"
